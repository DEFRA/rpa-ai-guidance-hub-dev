...
async def pull_repo(repo: str, repos_path: pathlib.Path):
    ...
    if not repo_path.exists():
        print(f"Skipping pull: repository directory not found for {repo} ({repo_path})")
        return

    if not (repo_path / ".git").exists():
        print(f"Skipping pull: {repo} at {repo_path} is not a git repository")
        return
    ...
    if process.returncode == 0:
        ...
        print(f"Failed to pull {repo}: {stderr.decode()}")
