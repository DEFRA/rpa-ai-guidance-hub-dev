...
- **`service-compose/*.yaml` filenames are load-bearing.** The clone/pull/update scripts derive each repo name by stripping `.yaml` from these filenames and cloning `https://github.com/DEFRA/<name>.git`. Adding a service means adding both a `service-compose/<repo-name>.yaml` and its `include` in `compose.yaml`. This is why third-party services we do not develop live in `dependencies/` instead — a `service-compose/cdp-uploader.yaml` would make the scripts clone the uploader's source.
